...
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get upgrade -yq \
  ...
  && wget https://download.foxtrot.blog/mongodb7/latest/armv8-a/libstdc%2B%2B6.deb && dpkg -i libstdc++6.deb && rm libstdc++6.deb \
  && wget https://download.foxtrot.blog/mongodb7/latest/armv8-a/mongodb-server.deb && dpkg -i mongodb-server.deb \
  && wget https://download.foxtrot.blog/unifi/latest/debian_universal/unifi_sysvinit_all.deb \
